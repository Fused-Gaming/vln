...
<h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold">
  // Hero heading: 28px → 36px → 48px → 56px

<h2 className="text-3xl sm:text-4xl md:text-5xl font-bold">
  // Section heading: 24px → 30px → 40px
...
### Component Spacing
```tsx
className="space-y-6 sm:space-y-8"  // Vertical stack
className="gap-4 sm:gap-6"          // Grid/flex gap
```
...
### Variants
```tsx
variant="primary"    // Sage background, white text
...
<CSSFade direction="up" delay={200}>
  <h1>Content fades in from bottom</h1>
</CSSFade>
```
...
<Counter end={47} duration={2000} suffix=" Vulns" />
```
...
**Features:**
- Futuristic circuit traces
- Animated electrical signals
- 3-color system (sage, blue, purple)
- Horizontal data buses, vertical interconnects
- IC components (chips, capacitors, resistors)
- 60fps performance
- 20% opacity overlay
...
2xl: 1536px  // Ultra-wide (unused, we cap at 1280px)
```
...
```tsx
// Standard container (1280px max, centered, padding)
<div className="container mx-auto px-4 sm:px-6">

// Narrow content (prose, forms)
<div className="max-w-2xl mx-auto">

// Wide content (stats grid, services)
...
### For Designers (Figma → Code)
1. Use VLN color tokens (not hex codes)
2. Follow 12px border radius (rounded-vln)
3. Use Lucide icons (not custom SVGs unless necessary)
4. Maintain 8px spacing grid
5. Test mobile responsiveness (320px minimum)

### For Developers (Code → Assets)
