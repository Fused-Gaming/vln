---
title: Color System
description: VLN WCAG AAA Compliant Color Palette — Sage Green, Sky Blue, Amber, Purple
---
...
**Accessibility:** Contrast on dark bg: **7.8:1 (AAA)**
...
- Link colors / secondary glows

**Accessibility:** Contrast on dark bg: **9.1:1 (AAA)**
...
**Accessibility:** Contrast on dark bg: **10.2:1 (AAA)**
...
**Accessibility:** Contrast on dark bg: **6.9:1 (AA+)**
...
**Usage:** Form validation, toast notifications, status indicators, alert banners.
...
|-------|-----|----------|-------|
| `vln-white` on `vln-bg` | `#f8f9fa` | 19.2:1 | AAA |
| `vln-gray` on `vln-bg` | `#cbd5e1` | 11.8:1 | AAA |
| `vln-gray-dark` on `vln-bg` | `#94a3b8` | 8.1:1 | AAA |
...
```css
--shadow-vln-glow:        0 0 12px #86d993   /* Sage */
--shadow-vln-glow-blue:   0 0 12px #7dd3fc   /* Blue */
--shadow-vln-glow-amber:  0 0 12px #fbbf24   /* Amber */
--shadow-vln-glow-purple: 0 0 12px #c084fc   /* Purple */
```

**Utility Classes:** `.glow-lift` `.glow-lift-blue` `.glow-lift-amber` `.glow-lift-purple`

:::note[Performance]
Glows use CSS `box-shadow` (GPU-accelerated). Only animate on hover/focus. Use `transition-all duration-300` for smooth effect.
:::
...
| Color | Hex | Use Case | Contrast | Level |
|-------|-----|----------|----------|-------|
| Sage | `#86d993` | Primary CTA, brand | 7.8:1 | AAA |
| Sky Blue | `#7dd3fc` | Secondary CTA, links | 9.1:1 | AAA |
| Amber | `#fbbf24` | Urgency, warnings | 10.2:1 | AAA |
| Purple | `#c084fc` | Premium features | 6.9:1 | AA+ |
| White | `#f8f9fa` | Primary text | 19.2:1 | AAA |
| Gray | `#cbd5e1` | Secondary text | 11.8:1 | AAA |
